...
app.config['MYSQL_HOST'] = os.environ.get('DB_SERVER')
...
# http://localhost:5000/pythinlogin/edit_profile- this will be the edit profile page, only accessible for loggedin users
@app.route('/pythonlogin/edit_profile')
def edit_profile():
    # Check if user is loggedin
    if 'loggedin' in session:
        # We need all the account info for the user so we can display it on the profile page
